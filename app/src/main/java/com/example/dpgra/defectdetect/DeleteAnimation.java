...
public class DeleteAnimation implements View.OnClickListener, Animator.AnimatorListener {
    ...
    public void onClick(View view) {
        if ( list instanceof PotholeList ) {
            ...
            ((View)view.getParent().getParent()).animate().alpha(0).setDuration(300).setListener(this);
            for ( int i = position + 1; i < list.size(); i++ ) {
                getViewByPosition(i).animate().translationYBy(-view.getHeight()).setDuration(200).setListener(this);
            }
            list.remove(position);
